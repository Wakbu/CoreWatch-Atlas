...
Windows·Linux Agent가 실제 지표를 수집하고 로컬에서 소비할 수 있는 단계다.
...
- 자동 테스트 37개: 계약 10, Agent 11, Linux 10, Windows 6
...
- 아직 중앙 Server/API와 Web UI가 없어 여러 장비의 통합 조회는 불가능하다.
...
다음 구현은 `docs/NEXT_STEPS.md`의 6단계 Server/API MVP다. 장비 등록·인증, Snapshot 수신, SQLite 이력과 온라인 판정을 별도 승인 후 진행한다.
