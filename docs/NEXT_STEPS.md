...
### 8C. 경고·배포·운영 (구현 완료: 2026-07-30)

1. Agent 수명주기 관리 (완료: 2026-07-29)
   - 관리자 전용 Agent 보관·복원·영구 삭제
   - 삭제 시 Snapshot 이력 보관 또는 함께 삭제 선택
   - 동일 장비 재등록 시 기존 오프라인 Agent 안내
   - 보관·복원·삭제 감사 로그
2. 경고 (완료: 2026-07-30)
   - 경고 규칙 영구 저장
   - 경고 확인·처리 상태 관리
   - HTTPS Webhook 알림 채널 연동
3. 배포 (구현 완료: 2026-07-30)
   - Server·Agent Windows·Linux 서비스 패키지
   - 컨테이너 이미지와 배포 설정
   - framework-dependent ZIP·SHA-256 Release 산출물 생성
4. 운영 안정화 (구현 완료: 2026-07-30)
   - SQLite와 Data Protection 키 백업·복구
   - 업데이트 절차
   - Docker 실구동과 대상 호스트 서비스 설치·부하·보안 검증은 Release 승인 전 수행
## 공통 완료 규칙
